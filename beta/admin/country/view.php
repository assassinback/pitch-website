...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'country WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'country WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'country ORDER BY name ASC';
