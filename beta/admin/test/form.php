...
if($id) {
	$act = 'update';
	$result = $db->query('SELECT * FROM ' . $dbPrefix . 'test WHERE id ='.$id);
	$data = $result->row_array();
}

$fields = array('title', 'description', 'video', 'url_slug', 'status');
					...
					<form action='' method='post' class="validateForm" enctype="multipart/form-data">
                        ...
                        <?php /*
                            ...
                            */ ?>
                            <div class="form-group">
                                <label>Name</label>
                                <input type='text' name='name' class='form-control' value="<?php echo $title; ?>" placeholder="Position name" maxlength="50" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter title name">
                            </div>
                            <div class="form-group">
