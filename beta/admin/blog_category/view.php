...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'blog_category WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'blog_category WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'blog_category ORDER BY title ASC';
