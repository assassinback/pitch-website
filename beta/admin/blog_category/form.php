...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($act == "update")
	{
		...
		{		
			...
			$where ="id ={$id}";
			updateData("blog_category",$data,$where);
...
if($id) {
	$act = 'update';
	$result = $db->query('SELECT * FROM ' . $dbPrefix . 'blog_category WHERE id ='.$id);
	$data = $result->row_array();
}

$fields = array('title', 'url_slug', 'status');
					...
					<form action='' method='post' class="validateForm" enctype="multipart/form-data">
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                <label>Name</label>
                                <input type='text' name='name' class='form-control' value="<?php echo $title; ?>" placeholder="Name" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter blog category name">
                            </div>
                            <div class="form-group">
                                <label>URL Slug</label>
                                <input type='text' name='url_slug' class='form-control' value="<?php echo $url_slug; ?>" placeholder="URL Slug" data-validation-engine="validate[required]" data-errormessage-value-missing="Please enter blog category url slug">
                            </div>
