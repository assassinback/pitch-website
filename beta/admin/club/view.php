...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'club WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'club WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'club ORDER BY name ASC';
