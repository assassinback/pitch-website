...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'admin WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'admin WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'admin ORDER BY name ASC';
                            ...
                            <table id="table-list" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                    <?php foreach ($result->result_array() as $row) { ?>
                                        <tr>
                                            <td><?php if ($row['super_admin'] != 1) { ?><input type="checkbox" name="delete[]" value="<?php echo $row['id']; ?>" ></td><?php } ?>
                                            <td><?php echo $row['name']; ?></td>
                                            <td><?php echo ($row['status']) ? 'Active' : 'Inactive'; ?></td>
                                            <td>
                                                <a href="<?php echo getAdminLink('add' . $module, 'id=' . $row['id']); ?>" class="btn btn-primary">Modify</a>
                                                <?php if ($row['super_admin'] != 1) { ?>
