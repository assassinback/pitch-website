...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'user WHERE user_type=2 AND id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM '.$dbPrefix.'user WHERE user_type=2 AND id IN ('.$delete.')');
        if ($result) {
            $_SESSION['msgType'] = 'danger';
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#table-list').DataTable( {
            ...
            "sAjaxSource": "<?php echo getAdminLink('coach/script.php', '', true);?>",
...
<div class="content-wrapper">
	...
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li><a href="#"><i class="fa fa-folder"></i>Coach</a></li>
		</ol>
	</section>
