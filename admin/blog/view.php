...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'blog_post WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'blog_post WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'blog_post ORDER BY id DESC';
                            ...
                            <table id="table-list" class="table table-bordered table-striped">
                                ...
                                <thead>
                                    <tr class="trheader">
                                        ...
                                        <th class="column-status1" width="25%">Status</th>
                                ...
                                <tbody>
                                    <?php foreach ($result->result_array() as $row) { ?>
                                        <tr>
                                            ...
                                            <td class="column-status1"><?php echo ($row['status']) ? 'Active' : 'Inactive'; ?></td>
                                            <td>
