...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'county WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'county WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'county ORDER BY name ASC';
