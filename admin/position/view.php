...
if($delId) {
    $result = $db->query('DELETE FROM ' . $dbPrefix . 'position WHERE id = ?', array($delId));
...
if(isset($_REQUEST['del']))
{
    ...
    if (count($delete) > 0) {
        ...
        $result = $db->query('DELETE FROM ' . $dbPrefix . 'position WHERE id IN ?', array($delId));
...
$query = 'SELECT * FROM ' . $dbPrefix . 'position ORDER BY name ASC';
