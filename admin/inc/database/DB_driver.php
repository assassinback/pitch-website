...
abstract class DATABASE_driver {
	 ...
	 * Queries list
	 *
	 * @see	DATABASE_driver::$save_queries
	 * @var	string[]
	 */
	 ...
	 * COUNT string
	 *
	 * @used-by	DATABASE_driver::count_all()
	 * @used-by	DATABASE_query_builder::count_all_results()
	 *
	 * @var	string
	 */
	...
	public function query($sql, $binds = FALSE, $return_object = NULL)
	{
		...
		{
			/*
			$filename = APP_ROOT . "/_logs/sql_".date('Ymd').".log";
	...
	{
		$driver = 'DATABASE_'.$this->dbdriver.'_result';

		if ( ! class_exists($driver, FALSE))
		{
			require_once(APP_ROOT.'/database/DB_result.php');
			require_once(APP_ROOT.'/database/drivers/'.$this->dbdriver.'/'.$this->dbdriver.'_result.php');
		}
	...
	public function display_error($error = '', $swap = '', $native = FALSE)
	{
		...
		foreach ($trace as $call)
		{
			if (isset($call['file'], $call['class']))
			{
				...
				if (strpos($call['file'], APP_ROOT.'/database') === FALSE && strpos($call['class'], 'Loader') === FALSE)
				{
					// Found it - use a relative path for safety
					$message[] = 'Filename: '.str_replace(array(SITE_URL, APP_ROOT), '', $call['file']);
