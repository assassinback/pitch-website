...
if (checkLogin()) {
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $arr=[];
                $user_id=$userScoreData["user_id"];
                $test_id=$userScoreData["test_id"];
                $weightage=$userScoreData["weightage"];
                $test="";
                if($test_id==3)
                {
                    $test="speed_score";
        ...
        if ($user_test_score->num_rows() > 0) {
            ...
            if ($user_test_score['total_score'] != $userScoreData['total_score'] || $user_test_score['weightage'] != $userScoreData['weightage']) {
                ...
                if($test!="")
                {
                    $query="UPDATE pitch_user_test_score_seperated set $test='$weightage' where user_id=$user_id";
                    $db->query($query);
                    echo "here";
                }
                // updateData('user_test_score_seperated', $userScoreData, 'id=' . $user_test_score['id']);
            }
        } else {
